...
- **8-кроковий опитувальник (Wizard)**: 
  - **Крок 1**: Збір та базова валідація контактних даних кандидата (ПІБ кирилицею, номер телефону у міжнародному/українському форматі з автоматичною нормалізацією, email).
  - **Кроки 2–7**: Динамічне формування переліку з 13+ типів документів (військовозобов'язаний, трудова книжка, освіта/студент денної форми, діти, інвалідність, ВПО, ЧАЕС тощо).
- **Пряме завантаження документів**: файлові віджети вбудовані безпосередньо в картки кожного документа (включаючи опціональне цифрове Фото 3×4 як альтернативу паперовим оригіналам).
- **Розумна валідація Резерв+/Оберіг**: перевірка наявності текстового шару виключно у цифрових PDF-витягах з Резерв+, виявлення сканів/фото, аналіз 5 ключових маркерів.
...
- **Атомарний Chunking електронної пошти**: автоматичне розбиття великого пакета документів на декілька листів (безпечний ліміт 14 MB raw на лист ≈ 18.8 MB MIME). Документи одного типу (`doc_id`) ніколи не розриваються між листами.
- **Зручна комунікація з кандидатом**: передача контактів (Телефон, Email) у шапці листа менеджеру та автоматичне встановлення заголовка `Reply-To` на email кандидата для швидкої відповіді з поштового клієнта.
...
- **Тестування**: pytest >= 9.1.1 (85/85 unit & integration тестів)
...
├── chunking.py            # Групування по doc_id, bin-packing, нумерація частин (≤14 MB)
...
hr_to = "hr@smart-solutions.ua, hr2@smart-solutions.ua" # Одне або декілька адрес через кому
...
Для запуску всіх 85 автоматичних тестів виконайте:
